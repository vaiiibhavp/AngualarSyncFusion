...
import { sampleData } from './datasource.service';
...
export class AppComponent implements OnInit {
  ...
  //   var mulData = [];
  //   for (var i = 0; i < this.treegrid.columns.length; i++) {
  //     if ((this.treegrid.columns[i] as any).index >= this.treegrid.frozenColumns) {
  //       mulData.push((this.treegrid.columns[i] as any).field);
  //     }
  //   }
  //   this.multiselect.dataSource = mulData;
  // }
  
  // removed(args:any){
  //   this.treegrid.getColumnByField(args.itemData).isFrozen = false ;
  ...
  //        if (args.itemData == this.treegrid.getVisibleColumns()[i].field) {
  ...
  contextMenuClick (args?: MenuEventArgs): void {
    ...
    {
      let selectedrowtodel = this.treegrid.getSelectedRows()[0] as HTMLTableRowElement
      this.sampleData.deletetask(selectedrowtodel.cells[2].innerHTML)
      .subscribe((data: any): void => {
  ...
  onrowSubmit(){
    ...
    if(this.rowform.status == 'INVALID')
    {
      ...
      if(this.rowIndex == 1){
        ...
        // add to json file 
        this.sampleData.addnewtask(value)
        .subscribe((data: any): void => {
          this.treegrid.addRecord(value, selectedrow+1); 
        })                    
      }
